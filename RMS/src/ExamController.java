...
public class ExamController {
	...
	ExamController()
	{
		...
		try {
			...
			ResultSet rs=SqlState.executeQuery("Select * from examreg where chairman like'%Yes%' and provost like'%Yes%'");		
			while(rs.next())
			{
			...
			Ap[o].addActionListener(new ActionListener()
			{
				public void actionPerformed(ActionEvent e)
				{
					...
					try {
						Connection SqlCon=DriverManager.getConnection(URL,username,password);
						Statement SqlState=SqlCon.createStatement();
						SqlState.executeUpdate("update  examreg set chairman='Yes' status='Yes' where sid like'%"+list[j]+"%' and yr like '%"+yrr[j]+"%' and sm like '%"+smm[j]+"%' chairman like'%Yes%' and provost like'%Yes%' and dept like'%"+A+"%'");	
