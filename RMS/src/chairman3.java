...
public class chairman3 {
	...
	chairman3(String S)
	{
		...
		try {
			...
			ResultSet rs=SqlState.executeQuery("Select * from examreg where chairman like'%No%' and provost like'%Yes%' and dept like'%"+A+"%'");		
			while(rs.next())
			{
			...
			Ap[o].addActionListener(new ActionListener()
			{
				public void actionPerformed(ActionEvent e)
				{
					...
					try {
						Connection SqlCon=DriverManager.getConnection(URL,username,password);
						Statement SqlState=SqlCon.createStatement();
						SqlState.executeUpdate("update  examreg set chairman='Yes' where sid like'%"+list[j]+"%' and yr like '%"+yrr[j]+"%' and sm like '%"+smm[j]+"%'");	
