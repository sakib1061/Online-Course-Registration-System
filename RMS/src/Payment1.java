...
public class Payment1 {
	...
	public static final String[] Tp={"Course","Exam"};
	...
	{	
		...
		JP.add(Semister);
		JLabel Type=new JLabel("Fee Type");
		Type.setBounds(350, 320, 100, 20);
		JP.add(Type);
		...
		JP.add(Sm1);
		final JComboBox Tp1=new JComboBox(Tp);
		Tp1.setBounds(460, 320, 200, 20);
		JP.add(Tp1);
		JButton Proceed=new JButton("Proceed");
		Proceed.setBounds(560,350,100,30);
		Proceed.setBackground(Color.cyan);
		Proceed.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
		JP.add(Proceed);
		JButton Back=new JButton("Back");
		Back.setBounds(460,350,100,30);
			...
			public void actionPerformed(ActionEvent e)
			{
				...
				try {
					...
					while(rs.next())
					{
						...
						cr=rs.getString("tcredit");
					}
					rs=rs=SqlState.executeQuery("Select * from examreg where sid like'%"+A+"%' and yr like'%"+i+"%' and sm like'%"+j+"%'" );	
					while(rs.next())
					{
						X=rs.getString("sid");
						Y=rs.getString("status");
					}
					rs=rs=SqlState.executeQuery("Select * from payments where sid like'%"+A+"%' and yr like'%"+i+"%' and sm like'%"+j+"%'" );	
					while(rs.next())
					{
						M=rs.getString("sid");
				...
				if(M.length()>0 && T.equals(Tp[k]))
				{
					Payment3 D=new Payment3(A,N);
					...
					if(Q.equals("Yes"))
					{
						double tc=Double.parseDouble(cr);
				Payment2 D=new Payment2(A,i,j,De,"Course",tc);
				...
				else if(k==1)
				{
					...
					if(Y.equals("Yes"))
					{
						double tc=Double.parseDouble(T);
				Payment2 D=new Payment2(A,i,j,De,"Exam",tc);
