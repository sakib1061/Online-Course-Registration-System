...
public class provost3 {
	...
	provost3()
	{
		...
		try {
			...
			ResultSet rs=SqlState.executeQuery("Select * from examreg where provost like'%No%' ");		
			while(rs.next())
			{
			...
			Ap[o].addActionListener(new ActionListener()
			{
				public void actionPerformed(ActionEvent e)
				{
					...
					try {
						Connection SqlCon=DriverManager.getConnection(URL,username,password);
						Statement SqlState=SqlCon.createStatement();
						SqlState.executeUpdate("update examreg set provost='Yes' where sid like'%"+list[j]+"%' and yr like '%"+yrr[j]+"%' and sm like '%"+smm[j]+"%'");	
